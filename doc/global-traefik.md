...
└── certs/
    ├── funkapparat.test.crt
    ├── funkapparat.test.key
...
http:
  routers:
    funkapparat-vite:
      rule: "Host(`funkapparat.test`)"
      entryPoints:
        - websecure
      service: funkapparat-vite-service
      tls: {} 

  services:
    funkapparat-vite-service:
      loadBalancer:
        servers:
...
tls:
  certificates:
    - certFile: /certs/funkapparat.test.crt
      keyFile: /certs/funkapparat.test.key
````
...
## 2. Connect a Project (Example: Funkapparat)
...
Navigate to the `certs` directory of your Master-Traefik and generate the certificates for your project. We include a wildcard (`*.`) so that subdomains like `dex.funkapparat.test` or `minio.funkapparat.test` are automatically covered by the same certificate:

````bash
cd ~/Projects/traefik-proxy/certs

mkcert -cert-file funkapparat.test.crt -key-file funkapparat.test.key "funkapparat.test" "*.funkapparat.test"
````
...
From now on, **any** domain ending in `.test` (e.g., `funkapparat.test`, `dex.funkapparat.test`, `anything.test`) will automatically resolve to your Master-Traefik without you ever having to touch a config file again!

> **Fallback:** If you cannot install dnsmasq, you will need to manually add every single subdomain like `127.0.0.1 funkapparat.test dex.funkapparat.test` to your `/etc/hosts` file.