...
Funkapparat supports OIDC-based authentication for the admin panel. The backend validates JWT Bearer tokens against a configurable OIDC provider (e.g., Dex, Keycloak, Auth0), and the React Admin frontend handles the login flow via `oidc-client-ts`.
...
auth:
  type: "oidc"
  name: "Dex"
  authority: "https://dex.funkapparat.test/dex"
...
3. **Callback**: After successful authentication, the provider redirects back to `https://funkapparat.test/auth-callback`. The `authProvider.handleCallback()` exchanges the authorization code for tokens.
